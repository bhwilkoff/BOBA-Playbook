...
- Per-section: horizontal scroll of `BOBAPriceTile` (M3 `Card` w/ `surfaceContainerLow`, thumb + price + source chip + `CustomTabsIntent` tap-through). Empty = `BOBAEmptyState` w/ refresh. Loading = 3-tile skeleton. The **`PriceTriGrid` composable** (M3 Surface + Row + `VerticalDivider`) renders `LOW / AVG / HIGH` for Recent Sales and Listed Range when count > 1; `PriceSingleAnchor` renders `FROM $X` / `LAST SOLD $X` for count == 1. Cell labels match iOS + web verbatim (DECISIONS.md #059 cross-platform vocabulary lock). Footer affordances: side-by-side `OutlinedButton`s — **eBay Sales** (`Intent.ACTION_VIEW` to `buildEbaySoldSearchUrl()`) + **View on Radish** (`Intent.ACTION_VIEW` to `card.radishUrl ?: homepage`, NOT `CustomTabsIntent`; DECISIONS.md #056). The slim catalog MUST carry `radishUrl` or the Android Radish link silently falls back to the homepage on every card — see DECISIONS.md #059 and `pipeline/scripts/regen_bundles.py`.
