...
## 065 — Daily pricing automation: GH Actions, not Cloudflare cron
*2026-05-29*

The post-#064 audit-driven calibration loop made the estimator improvable PER REBUILD; the open question was how to drive rebuilds without manual intervention. The answer: a daily GitHub Actions workflow (`.github/workflows/pricing-daily-refresh.yml`) that refreshes stale tracker data, rebuilds the artifact, runs the 9-audit framework, tracks history, gates regressions, and emits calibration recommendations. End-to-end documentation: `PRICING_AUTOMATION.md`.

**Why GH Actions, NOT a Cloudflare Worker cron** (the obvious alternative). Three load-bearing constraints push the work off the Cloudflare side:

1. **Free-plan limits** — a Worker cron has a 50-subrequest-per-invocation cap (the same wall that pushed the tracker to the push model in #058) and a 10ms-CPU cap on free tier. The daily refresh makes ~1,700 sequential HTTP calls (refresh + crawl) and runs a Python pipeline on 17,974 cards with statistical analysis. That's structurally not a Worker workload.
2. **D1 reads from compute** — `build_price_estimates.py` runs `wrangler d1 execute` to pull the priced pool; that's a CLI tool that needs a real runtime environment, not a Worker isolate.
3. **Git commits** — the artifact ends as a file in the repo (so GitHub Pages serves it to the Worker's `ESTIMATES_ARTIFACT_URL`). A Worker can't `git push`; a GH Action can.
