...
## 065 — Daily pricing automation: local launchd cron, NOT GH Actions, NOT Cloudflare Worker cron
*2026-05-29*

The post-#064 audit-driven calibration loop made the estimator improvable PER REBUILD; the open question was how to drive rebuilds without manual intervention. The answer: a daily **local launchd cron on Ben's Mac** (`scripts/com.bobaplaybook.pricing-daily.plist` + `scripts/daily_pricing_refresh.sh` + `scripts/install_pricing_cron.sh`) that refreshes stale tracker data, rebuilds the artifact, runs the 9-audit framework, tracks history, gates regressions, emits calibration recommendations, and `git push`es the new artifact. End-to-end documentation: `PRICING_AUTOMATION.md`.

**Two alternatives rejected**, in order of consideration:
...
**GitHub Actions** — viable but adds a credential. The pricing scripts use `wrangler d1 execute` to read the tracker D1, which authenticates via wrangler's local OAuth token cache (`~/.config/.wrangler/config/`). On a fresh ubuntu-latest runner that cache doesn't exist — wrangler in CI requires a `CLOUDFLARE_API_TOKEN` env var. Creating a new minimum-scope token + adding it as a repo secret is a 5-minute setup, but it's a NEW credential to rotate, audit, and secure. Ben's preference: don't add a credential when the existing local auth already works.

**Why local launchd is the right answer**:

1. **Zero new credentials**. The cron uses (a) wrangler's existing OAuth — already logged in via `wrangler login` from before this project started — and (b) macOS Keychain git credentials — already cached from daily `git push` activity. Nothing new to manage.
2. **Same shell, same wrangler, same Python**. The script is the exact same code Claude (and Ben) run interactively during development. No "works locally, breaks in CI" class of bugs — the environments are identical because they ARE the same environment.
3. **Predictable schedule**. launchd's `StartCalendarInterval` fires at 09:00 local; if the Mac is asleep, the missed run fires on next wake (launchd's normal queue semantics). Notifications + log files give immediate signal.
4. **Failure visibility**. macOS notifications + log files (`~/Library/Logs/boba-pricing-daily.{out,err}.log`) give immediate signal; no need to check a GitHub Actions tab.

**Trade-off accepted**: the cron only fires when the Mac is awake. For multi-day travel, the run queues once and fires on wake; for indefinite away time, manual trigger via `scripts/install_pricing_cron.sh --trigger` backfills. If this trade-off ever becomes uncomfortable, PRICING_AUTOMATION.md §7 documents the cloud-failover path (port `daily_pricing_refresh.sh` to a `.yml`, add `CLOUDFLARE_API_TOKEN`); the scripts are portable, only the orchestration is local.
