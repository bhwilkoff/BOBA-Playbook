...
**GitHub repo:** `BOBA-Playbook` · **Xcode product name:** `BOBAPlaybook` (hyphens not valid in Xcode product names)
...
- **Live web URL:** `https://bhwilkoff.github.io/BOBA-Playbook/`
...
| `bazookavault-images/` | Source images, already processed |
...
2. Name the repo: **`BOBA-Playbook`**
3. Clone locally: `git clone https://github.com/bhwilkoff/BOBA-Playbook`
4. In **GitHub Settings → Pages**: source = `main` branch, root `/`
5. Verify GitHub Pages is live at `https://bhwilkoff.github.io/BOBA-Playbook/`
...
GitHub repo / Xcode product name: BOBA-Playbook / BOBAPlaybook
Tagline: "Search. Scan. Collect. Play."
...
2. Product Name: **`BOBAPlaybook`** (no hyphens in Xcode — matches BOBA-Playbook repo)
...
7. Move starter files from `ios/` into `BOBAPlaybook/` group in Xcode:
   - `ios/App/AppNameApp.swift` → rename `BOBAPlaybookApp.swift`
...
10. Commit: `git add -A && git commit -m "Init Xcode project: BOBA Playbook (BOBAPlaybook)"`
...
mkdir -p /path/to/BOBA-Playbook/assets/data
mkdir -p /path/to/BOBA-Playbook/docs

cp unified-cards/data/cards.json        /path/to/BOBA-Playbook/assets/data/cards.json
cp unified-cards/data/cards-slim.json   /path/to/BOBA-Playbook/assets/data/cards-slim.json
cp unified-cards/data/categories.json   /path/to/BOBA-Playbook/assets/data/categories.json
cp unified-cards/data/search-index.json /path/to/BOBA-Playbook/assets/data/search-index.json

cp unified-cards/docs/CARD_SCHEMA.md       /path/to/BOBA-Playbook/docs/CARD_SCHEMA.md
cp EBAY_SEARCH_FINDINGS.md                 /path/to/BOBA-Playbook/docs/EBAY_SEARCH_FINDINGS.md
cp IMAGE_SOURCING_STRATEGY.md              /path/to/BOBA-Playbook/docs/IMAGE_SOURCING_STRATEGY.md
cp CLOUDFLARE_R2_SETUP.md                  /path/to/BOBA-Playbook/docs/CLOUDFLARE_R2_SETUP.md
```
...
```swift
// BOBA-Playbook/Models/Card.swift — decoded from cards-slim.json, cached in SwiftData
...
// BOBA-Playbook/Models/UserCard.swift — synced with Supabase user_cards table
...
// BOBA-Playbook/Models/Deck.swift
...
Display name: BOBA Playbook. GitHub repo: BOBA-Playbook. Xcode product name: BOBAPlaybook (hyphens not valid in Xcode).
Design language: retro-futurism + cyberpunk UI + glassmorphism.
...
When you start your first Claude Code session in `BOBA-Playbook`:
...
### ✅ Completed in Cowork
- Full card database scraped (Bazookasvault, Radish, GCS, eBay): **17,793 cards**
...
| **Fork DualAppTemplate → BOBA-Playbook** | 🔴 Must-do first | Step 1 above |
...
| Source | Cards | Notes |
|---|---|---|
| Bazookasvault (BV) | 4,756 | Highest res (745×1040), all sets |
