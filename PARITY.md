...
| Custom Rainbows | ✅ | ✅ | ⏳ M2 polish — read + create + edit + delete | Per-user filter goals; Supabase `user_custom_rainbows`. Web shipped tick 7 read-only display + tick 15 name editor + tick 16 full 7-dimension filter sub-pickers + Inspired Ink toggle + live progress preview. Android shipped read + create + delete previously; tick 59 added update (rename + criteria edit) at the repo + ViewModel layer — editor UI surface pending. |
