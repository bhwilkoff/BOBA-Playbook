...
| Whatnot active asks (Buy Now, NOT in waterfall) | ✅ | ⏳ | ⏳ | PRICING_PLAYBOOK §4 (Tier 2). `boba-ebay-proxy /whatnot/products` returns current active listings; Hybrid surfacing (this card's matches first, then "Other {hero}"). Asking signal only — never the sold waterfall/Market Est. Web shipped; iOS/Android client tiles pending. |
