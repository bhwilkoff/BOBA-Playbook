...
| Export deck (CSV file + text) | ✅ | ✅ | ✅ | **Format selector (2026-06-14): Full deck (default) vs Plays-only.** Full = v2 CSV (`id,name,type,release,number,cost,dbs,ability,bonus`; type = `HERO/HD/PL/BPL`) carrying Heroes + Hot Dogs + Plays + Bonus, round-trips into BOBA. Plays-only = legacy v1 slot CSV (`Slot,Card#,Name,Cost,Ability,DBS`) for external deck builders (deck-builder.bobattlearena.com) that take only play cards. iOS = segmented Picker (sticky `@AppStorage`); web = checkbox in export panel; Android = two menu items (`Export full deck` / `Export plays only`). Android gained the v1 slot exporter (was v2-only) for parity. |
| Import deck (CSV) | ✅ | ✅ | ✅ | Handles both v1 (slot) + v2 (`id,…`) on all three. All route Heroes + Hot Dogs + Plays + Bonus back into their sections (the resolved catalog Card carries its `cardType`). Resolution chain matched 2026-06-14: Android now resolves **bobaId-exact first** (was cardNumber-only → wrong variant on bobaId-v3 cardNumber collisions, DECISIONS.md #057), falling back to cardNumber (raw + `A - ` prefix strip) then name — same precedence as iOS `importDeckCSVv2` / web `_importCSVv2`. |
...
| Pricing panels (Buy Now + Sold) | ✅ | ✅ | ✅ | DECISIONS.md #013 + #058 + **#059**. Unified provenance-honest resolver `marketValue()` on all three (web `js/app.js` · iOS `PricingService.marketValue` · Android `marketValue()` in `:core:network`) — ONE place ranks the four signals: Recent Sales → Listed Range → Buy Now → Estimate. **Locked vocabulary (DECISIONS.md #059):** section headers `RECENT SALES` / `LISTED RANGE` / `MARKET EST.`; tri-grid cell labels `LOW / AVG / HIGH` everywhere (iOS no longer uses `EST. MID`); single-anchor cells `FROM $X` (asking) / `LAST SOLD $X` (transacted); footer buttons `eBay Sales` + `View on Radish` with verbatim-shared URL builders on all three. Android tri-grid shipped via `PriceTriGrid` Material 3 composable (M3 Surface + Row + VerticalDivider). |
...
| Radish recent sales | 🚫 | 🚫 | 🚫 | Removed 2026-05-23 per DECISIONS.md #056 + RADISH_REMOVAL_LOOP.md. Per-card "View on Radish" external link retained on all three platforms (homepage fallback when `card.radishUrl` is null). Android catalog was silently dropping `radishUrl` from the slim bundle until 2026-05-28 (DECISIONS.md #059) — fixed via `regen_bundles.py` NEW FIELD treatment so Android now resolves to the per-card URL like iOS + web. |
