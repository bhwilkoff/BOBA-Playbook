...
### 6.5 Bootstrapping estimator DATA without sold comps (shipped 2026-05-27)

The estimator *logic* (§6.2) was ready long before it could produce anything: diagnosed 2026-05-27, the tracker held priced data for **60 of 17,974 cards** — 59 "base", 45 FIRE — because the push model only collects what users view. You cannot build an honest per-card rarity estimate from 60 skewed cards: most rarity buckets are empty, so estimates collapse to a meaningless global average (the lone Glow battlefoil at $155 vs base FIRE commons at ~$5 shows the variance erased). The blocker was never the model — it was **data coverage**. Two offline tools fix it without Radish and without sold-comp API access (which we'll never have, §0):

- **`scripts/crawl_active_listings.py`** — walks the whole catalog through the *same* eBay proxy the app uses; the proxy's push model records each card's active listings into the tracker. **Stratified** (round-robins across treatment-family × weapon so buckets fill evenly), **resumable** (cursor file), **quota-safe** (sequential, ~800/run default, under eBay's ~5000/day Browse cap). Runs locally (no Cloudflare free-plan 50-subrequest cap — the same wall that forced the tracker's push model makes a catalog-grinding Worker cron impossible). Most catalog cards have *no* active eBay listings at any moment (thin market), so coverage is measured **per rarity bucket**, not per card — the majority of cards only ever get an estimate via comparables.

- **`scripts/build_price_estimates.py`** — joins the tracker's priced cards to the catalog and writes `assets/data/price-estimates.json`: for each card, the median of its **tightest rarity bucket** with ≥3 *other* priced cards, walking L1 `treatment×weapon×power×type` → L2 `treatment×weapon×power` → L3 `treatment×weapon`. **Honesty floor: L3 (a real rarity class) is the loosest level emitted** — never weapon-only or family-only, which would collapse a serialized chase card and a base common onto one median (#058). Sold comps preferred per card; asks used only with a `SOLD_HAIRCUT` (0.82). Robust **interquartile** range (p25/median/p75), not min/max, so a single mispriced listing can't blow the band. No "high" confidence while ask-derived. Comp-less rarity classes honestly emit **nothing** until the crawl fills their bucket.

**Serving — zero client changes.** The `boba-price-estimator` Worker's `/estimate?bobaId=` (which web/iOS/Android already call as the comp-less fallback) now falls back to this static artifact (`ESTIMATES_ARTIFACT_URL`, edge-cached + per-isolate-memoized) when its comp-derived KV is empty, mapped to the same response shape. So all three platforms get the baseline estimate with no client edits and no 3-platform bucket-logic drift. It still only displays where nothing better exists (Recent Sales / Listed Range outrank it, #058).
...
Free-tier budget per run (verified): ~1,200 eBay Browse calls (out of 5K/day free), ~25 GH Actions minutes (unlimited on public repos), ~5 D1 queries, ~5MB storage growth/month. One repository secret needed: `CLOUDFLARE_API_TOKEN` (`D1:Read` on `boba-pricing`). Full end-to-end documentation: [PRICING_AUTOMATION.md](./PRICING_AUTOMATION.md). Architecture decision: DECISIONS.md #065.
