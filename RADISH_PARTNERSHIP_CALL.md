...
- Without Radish art coverage, those 8,386 cards would have no canonical image — many of them are long-tail variants Bazooka Vault and eBay don't cover.
...
- **47% of catalog image coverage** evaporates. Either we go re-scrape eBay completed listings + BazookaVault (expensive, lower quality, slower), or 8,000+ cards regress to placeholder images.
...
- **BazookaVault CSV has 23,660 real-image rows on disk; we currently consume 5,877** — 4× headroom inside an asset we already own. Just rerunning `stage_a_scrape_bv.py` against the 8,386 bobaIds expects 70-85% hit rate.
