...
- [ ] **Run the BV image-backfill against the 8,386 Radish-sourced cards.** `assets/data/radish_backfill_queue.json` was generated by `scripts/identify_radish_sourced_cards.py` (re-run it any time to refresh). Feed the queue into `pipeline/scripts/stage_a_scrape_bv.py` — walk-away §8.1 expects 70-85% hit rate from BazookaVault alone. Folding results back updates each card's `imageSource` from RADISH → BV. Multi-hour job; needs your local environment to execute.
