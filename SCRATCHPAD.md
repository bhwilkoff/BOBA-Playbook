...
- **iPad first-class pass — PR 1+2+3** (DESIGN.md §6.6 + §6.6.1 + §6.6.2, ratified binding). Phone path untouched.
  ...
  - **Deferred to follow-up PR:** DecksView 2- or 3-column `NavigationSplitView` for iPad regular (DESIGN.md §6.6 — saved decks | pool | editor). Existing pool + fullScreenCover editor structure works on iPad but isn't first-class shape; refactor blocked on careful editor extraction (1400+ line file, deep `self`-references). Track here.
