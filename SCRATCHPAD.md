...
- **Last session**: 2026-04-12 — Set taxonomy overhaul + Discord auth fixes + admin panel. (Cowork) Complete set taxonomy rewrite: replaced coarse "Alpha"/"Griffey" set names with collector-facing product names (Alpha Edition, Alpha Update, Alpha Blast, Griffey Edition, National Starter Set, World Champions, Superfan Series, Promo Cards, Tecmo Bowl Edition, Big League Chew). `reconcile_all.py` now uses 3-tier resolution: Radish set names (primary) → BV set+sub_set mapped → variation inference fallback. All data files regenerated and deployed. See COWORK.md [2026-04-12] entry for full details. Previous session (Claude Code): Discord auth fully fixed (both flows) + admin panel user management added. (1) Discord Trade Room PKCE auth: both web (`js/discord.js`) and iOS (`DiscordService.swift`) were calling Discord's token endpoint directly without `client_secret` — Discord's app is a confidential client. Fixed by adding `POST /discord/token` endpoint to the Cloudflare Worker that proxies the exchange with `DISCORD_CLIENT_SECRET`. Worker secret set via wrangler. (2) Discord "Sign in with Discord" (Supabase OAuth): Supabase Dashboard had the old client secret; corrected to the value from Discord Developer Portal. (3) Admin panel user management: new Supabase RPC `get_admin_user_stats()` (SECURITY DEFINER) joins `auth.users` to expose last sign-in, display name, collection count, and total value — admin-only enforced inside the function. Both web (`js/collection.js`, `js/api.js`) and iOS (`AdminPanelView.swift`, `SupabaseClient.swift`, `UserCard.swift`) updated to show full user rows. (4) Hot dog image fix: `HD-1` (Dirty Water Dan) has no CDN image; Play tab substitution section now uses Frank (`HD-10_Frank_HotDog.webp`). (5) iOS 26 `Text` deprecation warnings fixed in `AdminPanelView.swift` — migrated from `Text(...) + Text(...)` to string interpolation. ⚠️ Pending manual action: update Supabase Dashboard → Authentication → Email Templates → Confirm signup with BOBA Playbook branding. ⚠️ Security: regenerate Discord client secret after confirming auth works.
- **Open questions**:
