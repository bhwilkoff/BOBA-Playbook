...
- **Last session**: 2026-04-09 — Hero name audit + bobaId plumbing completion. Applied 458 name-field corrections across cards.json and all 6 downstream bundles (Brockness/McArmyKnife class of bug resolved). Hardened reconcile_all.py with name normalization final step. Plumbed bobaId through ebay_review_server.py (listing map, variety map, UI, /decide payload) and Supabase card_corrections/card_image_overrides tables. Built new boba-ocr-audit Swift CLI that mirrors iOS CardScanner pipeline with NFKD + Cyrillic/Greek homoglyph fold and Bonus Plays exclusion. Final 14,680-card run: 14,381 MATCH (97.96%) / 265 UNCERTAIN / 34 DISAGREE — zero real catalog errors, all remaining noise is 6 kanji cards or short-hero-name foil-banner collisions (JPEG, Dr. J, XL, Gar). Baseline frozen at scripts/boba-ocr-audit/hero_name_audit/baseline_2026-04-09.csv for future regression detection. Also: bobaId rollout complete on web (js/collection.js, js/app.js, js/api.js); search-index.json rebuilt from card fields; iOS image navigation fix (CardDetailView .id(card.id)).
- **Open questions**:
