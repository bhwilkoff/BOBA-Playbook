...
- **CollectionView NavigationSplitView** — sidebar lens picker (My Cards / Rainbow / Shows) feeding detail. Existing `authenticatedView` + `path` + multiple `navigationDestination`s makes the refactor non-trivial; designation segmented Picker would stay inside the My Cards lens. Existing single-column with Rainbow/Shows in overflow Menu still works fine on iPad.
- **3-column DecksView** — saved-decks sidebar | pool | editor. Requires hoisting `loadDeck` from `DeckManagementSheet` to `DeckBuilderStore` so the new sidebar + the existing sheet share the load path. iPad portrait has tight per-column widths (~340pt × 3) which cramps the 5-col pool grid; iPad landscape works. Re-evaluate with usage data.
