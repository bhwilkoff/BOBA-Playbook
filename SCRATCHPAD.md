...
| Play Mode — Deck Builder | ✅ | ✅ | Templates + saved decks, format-aware (Standard/SPEC/SPEC+/Limited). |
| Play Mode — Practice setup UI | ✅ | ✅ | 3-tab layout (Game Mode / Your Deck / CPU Deck) + custom rules disclosure + format-compliance banner. Ported to web 2026-04-25. |
| Play Mode — Practice engine (executor) | ✅ | ✅ | All ops in sync: `versus_dice_roll`, `dice_gate`, `install_persistent`, `player_choice`, `reveal_play_for_conditional_free`. Source-card threading + auto-discard intent + structuredHandled fix all on both. |
| Play Mode — Random Deck (realistic) | ✅ | ✅ | Composition triad (8 recovery / 8 buffs / 4 utility / 4 denial / ≤6 bonus), power-curve tiers (50% high / 40% mid / 10% low), 6-per-power + 4-per-hero caps, format compliance. |
| Play Mode — Hero pulse animation | 📱 | ✅ | iOS-only — large active battle card scales + glows on power change. Web playmat is 7 mini columns where the visual would be lost. |
| Play Mode — DiceCoinReveal versus overlay | 📱 | ✅ | iOS shows full-screen YOU vs CPU dice columns with VS pip; web surfaces the same outcome via `pmEnqueueNotification` ("Versus roll: you 4, opponent 6 — OPPONENT WINS the roll"). |
| Play Mode — Tap-to-review on contributions | 📱 | ✅ | iOS breakdown panel rows open PlayReviewSheet with the played card's image. Web has a different breakdown surface; deferred. |
| Play Mode — Active battle layout fixes | 📱 | n/a | Hard-fixed 92pt panel + chip-strip-after-resolution apply only to iOS's large active card layout; web's mini-column playmat doesn't need them. |
| Play Mode — First-run hint banners | 📱 | ✅ | iOS HintsManager + 8 teaching moments. Web has the practice tutorial flow already from earlier session. |
