...
backfill those 8,386 cards from non-Radish sources (the card source
...
Output:
  - assets/data/radish_backfill_queue.json — bobaId + cardNumber + hero
    ...
    pipeline/scripts/stage_a_scrape_src.py (which already operates on
    a queue input) and by the future eBay image sourcer.

How to use:
  1. Run this script — generates the queue JSON.
  2. Feed the queue into stage_a_scrape_src.py (BV CSV has 4× headroom
